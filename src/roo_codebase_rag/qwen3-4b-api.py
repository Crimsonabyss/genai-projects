...
Optimized for lightweight embedding tasks with 2560 dimensions
"""
...
MODEL_CONFIG = {
    "model_name": "qwen3-embedding-4b",                 # Local optimized model, served by Ollama. don't add the :latest suffix
    # "model_name": "qwen3-embedding-4b",  # <-- doesn't work
    "dimensions": 2560,                                 # set the dimension size to 2560
    "max_context_length": 32768,                        # Qwen3-Embedding-4B has a context length of 32,000 tokens (32K).
    "temperature": 0.0,
    "supports_instructions": True,
    "supports_mrl": True,                               # Matryoshka Representation Learning
    "available_dimensions": [512, 768, 1024, 2560],     # MRL supported dimensions
    "quantization": "Q4_K_M",                           # Quantization set to q4_k_m
    "size_mb": 600,
    "use_case": "Instruction-aware embedding with MRL support",
    "performance_improvement": "1-5% with task-specific instructions",
    "cache_dir": "embedding_cache_4b_Q4_K_M"
}
...
class EmbeddingRequest(BaseModel):
    ...
    dimensions: Optional[int] = Field(
        ...
        description="Output dimensions (512, 768, 1024 or 2560 for MRL support)"
    )
...
class Qwen3_EmbeddingAPI:
    """Qwen3-Embedding API wrapper with optimal configurations"""
    ...
    def _apply_mrl_truncation(self, embedding: List[float], target_dimensions: int) -> List[float]:
        ...
        Qwen3-4B-Embedding supports 512, 768, 1024 and 2560 dimensions, and 8b can do 4096
        """
...
app = FastAPI(
    title="qwen3",
    description="OpenAI-compatible API for Qwen3-Embedding model, support 0.6b and 4b",
    version="1.0.0"
)

# Initialize API
embedding_api = Qwen3_EmbeddingAPI()

@app.get("/")
async def root():
    """API information"""
    return {
        "message": "Qwen3-Embedding API for 0.6b, 4b and 8b - RooCode Compatible ",
...
if __name__ == "__main__":
    print(f"🚀 Starting {MODEL_CONFIG['model_name']}-{MODEL_CONFIG['quantization']} API server...")
